...
namespace iDeal.Tests.Unit
{
    class SignatureProviderTests : TestFixture
    {
        ...
        {
            var signatureProvider = new SignatureProvider(Certificate, BankCertificate);
        ...
        {
            var signatureProvider = new SignatureProvider(Certificate, BankCertificate);
            Assert.AreEqual(Certificate.Thumbprint, signatureProvider.GetThumbprint());
        }

        [Test]
        public void CanGetThumbprintBankCertificate()
        {
            var signatureProvider = new SignatureProvider(Certificate, BankCertificate);
            Assert.AreEqual(BankCertificate.Thumbprint, signatureProvider.GetThumbprintBankCertificate());
        }
    }
}
